...
BRAT normally tracks the current release. To reproduce a report against a specific version, run **BRAT: Add a beta plugin with frozen version based on a release tag**, enter `K0g1/Templar`, and enter the exact release tag, for example `1.2.0-alpha.2`. A frozen plugin does not follow later updates automatically.
